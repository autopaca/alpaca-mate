...
import {
  ...
  PoolType,
  PositionStatus,
  PositionStatusAfterClose,
  RelativeInfo,
} from 'Src/Calculator/type.d';
import { aprToApy } from 'Calculator/utils';
import { cakeMaxiAssets } from 'Src/poolMetas';

export const poolTypeState = atom<PoolType>({
  key: 'poolType',
  default: PoolType.LP,
});
...
// ------------------------------ CAKE PCS Syrup ---------------------------------------
export const cakeMaxiPoolState = atom<CakeMaxiPoolInfo | undefined>({
  key: 'cake-maxi-pool-info', // unique ID (with respect to other atoms/selectors)
  default: {
...
export const cakeMaxiAssetsState = selector<string[] | undefined>({
  key: 'cake-maxi-assets',
  get: ({ get }) => {
    const pool = get(cakeMaxiPoolState);
    if (pool) {
      return [pool.borrowableAssets[pool.borrowIndex], 'CAKE'];
    }
  },
});

const caseMaxiFarmInputSelector = selector<FarmInput | undefined>({
  key: 'case-maxi-farmInputSelector',
  get: ({ get }) => {
    const pool = get(cakeMaxiPoolState);
    if (pool) {
      const leverage = pool.leverage;
...
export const cakeMaxiBorrowedState = selector<Borrowed | undefined>({
  ...
  get: ({ get }) => {
    ...
    const assetsValue = get(cakeMaxiAssetsValueState)!;
    const borrowedAmount = ((farmInput.leverage! - 1) * assetsValue[0]).toString();
    const borrowedPrice = farmInput.assetDetails[0].price;
    const leverage = farmInput.leverage ?? 1;
    const borrowedValues = calculateValues(
      parseFloat(borrowedAmount) * (borrowedPrice ?? 0),
      farmInput.assetDetails[0].price,
...
export const cakeMaxiPositionAtOpenState = selector<PositionStatus | undefined>({
  ...
  get: ({ get }) => {
    ...
    const borrowed = get(cakeMaxiBorrowedState)!;
    const assetsValue = get(cakeMaxiAssetsValueState)!;
    const borrowedPrice = borrowed.price || 0;
    const borrowedAmount = borrowed.borrowedAmount || '0';
    const totalInBusd = parseFloat(borrowedAmount) * borrowedPrice + assetsValue[2];
